  ...
  const fetchLogs = async () => {
    try {
      ...
      const processedData = Array.isArray(data)
        ? data
          .filter(call => {
              // Vapi's /call list API often strips the deep 'metadata' object to save bandwidth.
              // We fallback to checking if the dynamically injected Name matches our pattern.
              const hasAppId = call.metadata?.appId === TARGET_APP_ID || call.assistant?.metadata?.appId === TARGET_APP_ID;
              const hasNameMatch = (call.name && call.name.startsWith('Call with')) || 
                                   (call.assistant?.name && call.assistant.name.startsWith('Call with'));
              
              // Only return calls that were uniquely generated by our React frontend
              return hasAppId || hasNameMatch;
          })
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 lg:p-10 font-sans">
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
        ...
        {/* Filters & Actions */}
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-gray-900/40 backdrop-blur-xl p-5 rounded-2xl border border-gray-800/50 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { label: 'Total Web Calls', value: logs.length, icon: PhoneCall, color: 'blue' },
            {
              label: 'Avg duration',
              value: logs.length > 0 ? formatDuration(logs.reduce((acc, curr) => acc + (curr.duration || 0), 0) / logs.length) : '0s',
              icon: Clock3,
              color: 'green'
            },
            {
              label: 'Successful Deals',
              value: logs.filter(l => l.analysis?.successEvaluation === 'true' || l.analysis?.successEvaluation === true).length,
              icon: CheckCircle2,
              color: 'purple'
            }
          ].map((stat, i) => (
